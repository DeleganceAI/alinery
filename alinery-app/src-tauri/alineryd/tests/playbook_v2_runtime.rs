...
use std::{fs, os::unix::fs::PermissionsExt, path::PathBuf, process::{Child, Command, Stdio}, thread, time::{Duration, Instant}};
use alinery_core::{daemon_client::{DaemonClient, UiControlConnection}, execution::{CompletionOutcome, ExecutionLifecycle}, task_creation::*, RUNNER_EVENT_PROTOCOL_VERSION};
use serde_json::{json, Value};

struct Fixture { root: PathBuf, child: Child, client: DaemonClient }
impl Fixture {
    fn new() -> Self {
        let root = PathBuf::from(format!("/tmp/al-v2-{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(root.join(".alinery")).unwrap();
        for args in [vec!["init", "-q"], vec!["config", "user.email", "fixture@example.invalid"], vec!["config", "user.name", "Fixture"], vec!["commit", "--allow-empty", "-qm", "initial"]] {
        ...
        fs::write(root.join(".alinery/harnesses.toml"), format!("[[harness]]\nkey='omp'\nname='Fixture'\nbinary='{}'\nargs=[]\nmodel_arg=[]\nprompt_injection='arg'\nadapter='omp'\n", harness.display())).unwrap();
        let child = Command::new(env!("CARGO_BIN_EXE_alineryd")).args(["--repo", root.to_str().unwrap(), "--app-config", root.join("app.toml").to_str().unwrap()])
            .env("ALINERY_RUNNER_PATH", &runner).env("ALINERY_HOST_EXECUTABLE", std::env::current_exe().unwrap()).stdout(Stdio::null()).stderr(Stdio::inherit()).spawn().unwrap();
        let client = DaemonClient { socket_path: root.join(".alinery/alineryd.sock") };
    ...
    fn state(&self) -> TaskExecutionReply { self.client.get_task_execution(&GetTaskExecutionRequest { task_slug:"fixture".into() }).unwrap() }
    ...
    fn outputs(&self, record: &alinery_core::execution::ExecutionRecord) {
        for output in &record.outputs { fs::write(self.root.join(".alinery/tasks/fixture/artifacts").join(&output.relative_path), "accepted evidence").unwrap(); }
    }
    fn release(&self, id: &str) { fs::write(self.root.join(format!("release-{id}")), "exit").unwrap(); }
}
impl Drop for Fixture {
    fn drop(&mut self) { let _ = self.client.call(&json!({"op":"shutdown"})); let _ = self.child.wait(); let _ = fs::remove_dir_all(&self.root); }
}
fn wait(mut condition: impl FnMut() -> bool) {
    let until = Instant::now() + Duration::from_secs(10);
    while !condition() { assert!(Instant::now() < until, "fixture condition timed out"); thread::sleep(Duration::from_millis(10)); }
}
fn definition() -> String {
    let mut source = "+++\nversion=2\nkey='fixture'\ntitle='Fixture'\ndescription=''\ndefault_model=''\ndefault_harness='omp'\n".to_string();
    for (key, inputs, output) in [("first", "[]", "one.md"), ("second", "[]", "two.md"), ("join", "[{path='one.md',mode='single'},{path='two.md',mode='single'}]", "joined.md")] {
        source.push_str(&format!("[[step]]\nkey='{key}'\ntitle='{key}'\nshort=''\ninputs={inputs}\noutputs=[{{path='{output}'}}]\nmodel=''\nharness=''\nis_coding_step=true\nauto_advance_default=false\n"));
    }
    source.push_str("+++\n");
    for key in ["first", "second", "join"] { source.push_str(&format!("<!-- alinery:step {key} -->\nWrite your assigned outputs.\n")); }
    source
}
...
fn held_roots_and_coding_capacity_are_durable_and_individual_start_isolated() {
    ...
    let started = fixture.client.start_session(&StartSessionRequest { task_slug:"fixture".into(), session_id:first.owner_session_id.clone() }).unwrap();
    assert_eq!(started.start, "started");
    assert!(!fixture.state().state.executions[&second.id].start_requested);
    let queued = fixture.client.start_session(&StartSessionRequest { task_slug:"fixture".into(), session_id:second.owner_session_id.clone() }).unwrap();
    ...
    wait(|| fixture.state().state.executions.values().any(|r| r.candidate.step_key == "join" && r.lifecycle == ExecutionLifecycle::Running));
}
...
fn only_retained_authenticated_ui_channel_grants_the_current_owner() {
    ...
    let first = created.executions.iter().find(|r| r.candidate.step_key == "first").unwrap();
    fixture.client.start_session(&StartSessionRequest { task_slug:"fixture".into(), session_id:first.owner_session_id.clone() }).unwrap();
    wait(|| fixture.root.join(format!("token-{}",first.owner_session_id)).exists());
    fixture.outputs(first);
    ...
    assert_eq!(fixture.client.call(&json!({"op":"status","id":first.owner_session_id})).unwrap()["agent"]["state"], "waiting_for_approval");
    let grant = AllowExecutionCompletionRequest { task_slug:"fixture".into(), execution_id:first.id.clone(), session_id:first.owner_session_id.clone() };
    ...
    let mut stale = grant.clone(); stale.session_id = "retired-owner".into();
    ...
    assert_eq!(fixture.event(&first.owner_session_id)["completion"]["status"], "invalid_outputs");
    assert_eq!(fixture.client.call(&json!({"op":"status","id":first.owner_session_id})).unwrap()["agent"], invalid_status["agent"]);
    fixture.outputs(first);
...
fn failed_start_is_retained_and_requires_explicit_stopped_owner_recovery() {
    let fixture = Fixture::new();
    let created = fixture.create(false, true);
    fs::write(fixture.root.join(".alinery/harnesses.toml"), "[[harness]]\nkey='omp'\nname='Missing'\nbinary='/nonexistent/alinery-fixture'\nargs=[]\nmodel_arg=[]\nadapter='omp'\n").unwrap();
    let first = &created.executions[0];
    let failed = fixture.client.start_session(&StartSessionRequest { task_slug:"fixture".into(), session_id:first.owner_session_id.clone() }).unwrap();
    assert_eq!(failed.start, "failed");
    assert!(failed.execution.as_ref().unwrap().shutdown_confirmed);
    assert!(fixture.client.start_session(&StartSessionRequest { task_slug:"fixture".into(), session_id:first.owner_session_id.clone() }).is_err());
    let recovered = fixture.client.create_execution_session(&CreateExecutionSessionRequest { task_slug:"fixture".into(), target:ExecutionSessionTarget::Primary { step_key:first.candidate.step_key.clone(), execution_id:Some(first.id.clone()),input_occurrence_ids:None }, launch_override:Some(alinery_core::LaunchChoices { harness: "omp".into(), model: "corrected-model".into() }),prompt_extra:None,start:false }).unwrap();
...
fn repeated_explicit_manual_runs_are_independent_and_do_not_replace_the_ordinary_binding() {
    ...
    let request = CreateExecutionSessionRequest { task_slug:"fixture".into(), target:ExecutionSessionTarget::Primary { step_key:"first".into(),execution_id:None,input_occurrence_ids:Some(Vec::new()) },launch_override:None,prompt_extra:None,start:false };
...
fn crash_after_acceptance_never_turns_an_unproven_owner_into_a_handoff() {
    ...
    let result = fixture.client.create_execution_session(&CreateExecutionSessionRequest {
        task_slug:"fixture".into(),target:ExecutionSessionTarget::Primary {step_key:"first".into(),execution_id:Some(first.id.clone()),input_occurrence_ids:None},launch_override:None,prompt_extra:None,start:false });
...
fn malformed_product_defaults_fail_before_task_or_git_provisioning() {
    ...
    assert!(!alinery_core::git_cmd(&fixture.root).args(["show-ref", "--verify", "--quiet", "refs/heads/invalid-config"]).status().unwrap().success());
}

#[test]
fn foreign_lane_cannot_create_auxiliary_sessions_on_an_owned_v2_task() {
    struct Lane { child: Child, client: DaemonClient }
    impl Drop for Lane {
        fn drop(&mut self) {
    ...
    let child = Command::new(env!("CARGO_BIN_EXE_alineryd"))
        .args(["--repo", fixture.root.to_str().unwrap(), "--app-config", fixture.root.join("app.toml").to_str().unwrap(), "--socket-namespace", "other"])
        .env("ALINERY_RUNNER_PATH", fixture.root.join("runner"))
        .env("ALINERY_HOST_EXECUTABLE", std::env::current_exe().unwrap())
        .stdout(Stdio::null()).stderr(Stdio::inherit()).spawn().unwrap();
    let other = Lane { child, client: DaemonClient { socket_path: alinery_core::alineryd_socket_path(&fixture.root, Some("other")) } };
    wait(|| other.client.version_checked().is_ok());
    let sessions = || fs::read_dir(fixture.root.join(".alinery/tasks/fixture/sessions")).unwrap()
        .map(|entry| entry.unwrap().file_name()).collect::<std::collections::BTreeSet<_>>();
    let before = sessions();
    let result = other.client.create_execution_session(&CreateExecutionSessionRequest {
        task_slug: "fixture".into(), target: ExecutionSessionTarget::Auxiliary {
            harness: "no-harness".into(), model: None, prompt: None,
...
fn replacement_configuration_cannot_start_an_existing_auxiliary_owner() {
    let mut fixture = Fixture::new();
    fixture.create(false, true);
    let auxiliary = fixture.client.create_execution_session(&CreateExecutionSessionRequest {
        task_slug: "fixture".into(), target: ExecutionSessionTarget::Auxiliary {
            harness: "no-harness".into(), model: None, prompt: None,
        }, launch_override: None, prompt_extra: None, start: false,
    }).unwrap();
    ...
    fixture.child.wait().unwrap();
    fixture.child = Command::new(env!("CARGO_BIN_EXE_alineryd"))
        .args(["--repo", fixture.root.to_str().unwrap(), "--app-config", fixture.root.join("different-app.toml").to_str().unwrap()])
        .env("ALINERY_RUNNER_PATH", fixture.root.join("runner"))
        .env("ALINERY_HOST_EXECUTABLE", std::env::current_exe().unwrap())
        .stdout(Stdio::null()).stderr(Stdio::inherit()).spawn().unwrap();
    wait(|| fixture.client.version_checked().is_ok());
    assert!(fixture.client.start_session(&StartSessionRequest {
        task_slug: "fixture".into(), session_id: auxiliary.session.id.clone(),
    }).is_err());
    let metadata: alinery_core::SessionMeta = serde_json::from_slice(&fs::read(
        alinery_core::session_meta_path(&fixture.root, "fixture", &auxiliary.session.id)
    ).unwrap()).unwrap();
...
fn terminal_sentinel_launches_a_shell_without_claiming_graph_capacity() {
    let fixture = Fixture::new();
    fixture.create(false, true);
    let reply = fixture.client.create_execution_session(&CreateExecutionSessionRequest {
        task_slug: "fixture".into(), target: ExecutionSessionTarget::Auxiliary {
            harness: "no-harness".into(), model: None, prompt: None,
    ...
    fixture.client.call(&alinery_core::write_request(&reply.session.id, &format!("printf shell-live > '{}'\r", proof.display()))).unwrap();
...
fn fanout_complete_waits_for_the_paused_worker_and_computes_assigned_results() {
    ...
    source.push_str("+++\n");
    for key in ["seed", "worker", "collect"] { source.push_str(&format!("<!-- alinery:step {key} -->\nUse only assigned inputs and outputs.\n")); }
    let request: CreateTaskRequest = serde_json::from_value(json!({
    ...
    wait(|| fixture.state().state.executions.values().filter(|record| record.candidate.step_key == "worker" && record.lifecycle == ExecutionLifecycle::Running).count() == 3);
    ...
    for worker in &workers {
        ...
        assert_eq!(PathBuf::from(fs::read_to_string(fixture.root.join(format!("cwd-{}", worker.owner_session_id))).unwrap().trim()).canonicalize().unwrap(), worktree);
        ...
        ui.allow_execution_completion(&AllowExecutionCompletionRequest { task_slug: "fixture".into(), execution_id: worker.id.clone(), session_id: worker.owner_session_id.clone() }).unwrap();
    ...
    ui.allow_execution_completion(&AllowExecutionCompletionRequest { task_slug: "fixture".into(), execution_id: paused.id.clone(), session_id: paused.owner_session_id.clone() }).unwrap();
    ...
    wait(|| fixture.state().state.executions.values().any(|record| record.candidate.step_key == "collect" && record.lifecycle == ExecutionLifecycle::Running));
    ...
    let answer: u64 = ids.iter().map(|id| fs::read_to_string(artifacts.join(&collected.state.occurrences[id].relative_path)).unwrap().parse::<u64>().unwrap()).sum();
    assert_eq!(answer, 38);
