...
pub enum ExecutionLifecycle { Queued, Starting, Running, Finishing, Completed, LaunchFailed, Failed, Interrupted }
...
pub enum ContextCause {
    Root,
    Member { collection_id: String, occurrence_id: String },
    Loop { component_steps: BTreeSet<String>, trigger_occurrences: BTreeSet<String> },
}
...
impl ExecutionCandidate {
    pub fn binding_key(&self) -> Result<String, String> {
        ...
        for ids in normalized.inputs.values_mut() { ids.sort(); ids.dedup(); }
...
pub fn execution_state_path(repo: &Path, slug: &str) -> Result<PathBuf, String> {
    if crate::safe_component(slug) != Some(slug) { return Err("invalid task slug".into()); }
...
pub fn read_execution_state(repo: &Path, slug: &str) -> Result<TaskExecutionState, String> {
    ...
    if state.version != 1 || state.max_live_sessions == 0 { return Err("unsupported or invalid task execution state".into()); }
    Ok(state)
}
...
pub fn read_task_playbook(repo: &Path, slug: &str, state: &TaskExecutionState) -> Result<NormalizedPlaybook, String> {
    ...
    if definition_identity(source.as_bytes()) != state.definition_identity { return Err("retained task definition integrity mismatch".into()); }
...
pub fn new_execution_state(reference: PlaybookRef, source: &str, owning_lane: String, max_live_sessions: u32, enabled_steps: BTreeSet<String>, launch_defaults: LaunchChoices) -> Result<TaskExecutionState, String> {
    ...
    if definition.key != reference.key { return Err("task playbook key does not match source identity".into()); }
    ...
    for key in &enabled_steps {
        if !definition.step.iter().any(|step| &step.key == key) { return Err(format!("unknown enabled step '{key}'")); }
    }
    let root = ExecutionContext { id: "root".into(), parent_id: None, cause: ContextCause::Root, bindings: BTreeMap::new() };
    Ok(TaskExecutionState {
        version: 1, revision: 0, creation: "provisioning".into(), creation_error: None,
        owning_lane, owning_app_config_identity: String::new(), definition_identity: definition_identity(source.as_bytes()), reference, max_live_sessions,
        enabled_steps, launch_defaults, initial_start_requested: false, contexts: BTreeMap::from([(root.id.clone(), root)]),
        executions: BTreeMap::new(), occurrences: BTreeMap::new(), collections: BTreeMap::new(),
    })
}
...
pub fn install_seed(state: &mut TaskExecutionState, logical_path: &str, relative_path: &str) -> Result<String, String> {
    ...
    if selector.wildcard { return Err("seed must have an exact logical path".into()); }
    let root = state.contexts.get_mut("root").ok_or("missing root context")?;
    if root.bindings.contains_key(logical_path) { return Err(format!("duplicate seed '{logical_path}'")); }
    let id = uuid::Uuid::new_v4().to_string();
    root.bindings.insert(logical_path.into(), vec![id.clone()]);
    state.occurrences.insert(id.clone(), ArtifactOccurrence {
        id: id.clone(), producer_execution_id: None, selector: logical_path.into(), logical_path: logical_path.into(),
        relative_path: relative_path.into(), depth: 0, discriminator: 0, context_id: "root".into(), collection_ids: BTreeSet::new(),
    });
    Ok(id)
}
...
pub fn mutate_execution_state<T>(repo: &Path, slug: &str, lane: &str, app_config_identity: &str, operation: &str, mutate: impl FnOnce(&NormalizedPlaybook, &mut TaskExecutionState) -> Result<T, String>) -> Result<T, String> {
...
pub fn resolve_execution_launch(definition: &NormalizedPlaybook, step: &NormalizedStep, task: &LaunchChoices, product: &LaunchChoices, explicit: Option<&LaunchChoices>) -> Result<LaunchChoices, String> {
    ...
    let launch = LaunchChoices {
        harness: first([explicit.map_or("", |v| v.harness.as_str()), &step.harness, &definition.default_harness, &task.harness, &product.harness, "omp"]),
        model: first([explicit.map_or("", |v| v.model.as_str()), &step.model, &definition.default_model, &task.model, &product.model]),
    };
    if launch.harness != "omp" { return Err(format!("unsupported graph harness '{}'", launch.harness)); }
    Ok(launch)
}
...
fn validate_execution_binding(state: &TaskExecutionState, step: &NormalizedStep, candidate: &ExecutionCandidate) -> Result<(), String> {
    ...
    for input in &step.inputs {
        ...
        for id in ids {
            ...
            if !selector.matches(&occurrence.logical_path) { return Err("input occurrence does not match declared selector".into()); }
        }
        match input.mode {
            ...
            InputMode::Each => {
                ...
                if ids.len() != 1 || candidate.each_member_id.as_ref() != ids.first()
                    || !collection.membership_closed || !collection.member_occurrence_ids.contains(&ids[0]) {
            ...
            InputMode::Complete => {
                uses_complete = true;
                let collection = state.collections.get(candidate.complete_collection_id.as_ref().ok_or("missing complete collection")?).ok_or("unknown complete collection")?;
                let members: BTreeSet<_> = collection.member_occurrence_ids.iter().filter(|id| {
                    state.occurrences.get(*id).is_some_and(|occurrence| selector.matches(&occurrence.logical_path))
                }).collect();
                if !collection.membership_closed || members != ids.iter().collect()
                    || collection.context_id != candidate.context_id
                    || collection.expected_execution_ids.iter().any(|id| !state.executions.get(id).is_some_and(|record| {
                        record.lifecycle == ExecutionLifecycle::Completed && record.shutdown_confirmed && record.receipt_id.is_some()
                    })) {
    ...
    if uses_each != candidate.each_collection_id.is_some() || uses_each != candidate.each_member_id.is_some()
        || uses_complete != candidate.complete_collection_id.is_some() {
...
fn occupied_artifacts(root: &Path, relative: &Path, paths: &mut Vec<String>) -> Result<(), String> {
    ...
    for entry in entries {
        ...
        if metadata.is_dir() && !metadata.file_type().is_symlink() {
            if relative.as_os_str().is_empty() && matches!(entry.file_name().to_str(), Some("attachments" | "subtasks")) { continue; }
            paths.push(name.into());
            occupied_artifacts(root, &rel, paths)?;
        } else { paths.push(name.into()); }
    }
    Ok(())
}
...
pub fn reserve_execution(repo: &Path, slug: &str, definition: &NormalizedPlaybook, state: &mut TaskExecutionState, candidate: ExecutionCandidate, product: &LaunchChoices, explicit: Option<&LaunchChoices>, start_requested: bool) -> Result<String, String> {
    let binding_key = candidate.binding_key()?;
    if !candidate.manual {
        if let Some(existing) = state.executions.values().find(|e| !e.candidate.manual && e.binding_key == binding_key) { return Ok(existing.id.clone()); }
    }
    let step = definition.step.iter().find(|step| step.key == candidate.step_key).ok_or("unknown execution step")?;
    if !state.contexts.contains_key(&candidate.context_id) { return Err("unknown binding context".into()); }
    ...
    for occurrence_id in candidate.inputs.values().flatten() {
        ...
        if !occurrence_deliverable(state, occurrence) { return Err("input producer has not completed handoff".into()); }
        parent_depth = parent_depth.max(occurrence.depth);
        if let Some(parent) = &occurrence.producer_execution_id { parents.insert(parent.clone()); }
    }
    ...
    let reservations: Vec<_> = state.executions.values().flat_map(|e| e.outputs.iter()).map(|a| ArtifactSelector::parse(&a.relative_path)).collect::<Result<_,_>>()?;
    ...
    for directory in &required_directories {
        ...
        if reservations.iter().any(|reservation| reservation.matches(directory)) {
            return Err(format!("output directory '{directory}' conflicts with a reserved output file"));
        }
    }
    let mut occupied_directories = parent_directories(state.executions.values().flat_map(|record| record.outputs.iter()).map(|assignment| assignment.relative_path.as_str()));
    ...
    for output in &step.outputs {
        ...
        loop {
            ...
            let physical = ArtifactSelector::parse(&relative_path)?;
            ...
            let reserved_overlap = reservations.iter().any(|other| physical.overlaps(other)) || outputs.iter().any(|a| ArtifactSelector::parse(&a.relative_path).is_ok_and(|other| physical.overlaps(&other)));
            if !existing_overlap && !reserved_overlap {
                outputs.push(OutputAssignment { selector: output.path.clone(), relative_path, discriminator: instance });
                break;
            }
    ...
    let record = ExecutionRecord {
        id: id.clone(), binding_key, candidate, outputs, parent_execution_ids: parents, depth,
        owner_session_id: format!("s{}", uuid::Uuid::new_v4()), previous_session_ids: Vec::new(), launch,
        is_coding_step: step.is_coding_step, start_requested, lifecycle: ExecutionLifecycle::Queued,
        permission: if state.enabled_steps.contains(&step.key) { CompletionPermission::Automatic } else { CompletionPermission::Locked },
        receipt_id: None, exit_code: None, shutdown_confirmed: false, error: None,
    };
...
pub fn occurrence_deliverable(state: &TaskExecutionState, occurrence: &ArtifactOccurrence) -> bool {
    match &occurrence.producer_execution_id {
        ...
        Some(id) => state.executions.get(id).is_some_and(|e| e.lifecycle == ExecutionLifecycle::Completed && e.shutdown_confirmed && e.receipt_id.is_some()),
    }
}
...
pub fn request_execution_start(state: &mut TaskExecutionState, id: &str) -> Result<(), String> {
    ...
    if record.lifecycle != ExecutionLifecycle::Queued { return Err("execution is not queued; recover failed work explicitly".into()); }
    record.start_requested = true;
    Ok(())
}

pub fn claim_execution_launch(state: &mut TaskExecutionState, id: &str) -> Result<bool, String> {
    if state.creation != "ready" { return Err("task creation is not ready".into()); }
    let record = state.executions.get(id).ok_or("unknown execution")?;
    if record.lifecycle != ExecutionLifecycle::Queued || !record.start_requested { return Ok(false); }
    let live = state.executions.values().filter(|e| e.lifecycle.holds_capacity()).count();
    if live >= state.max_live_sessions as usize { return Ok(false); }
    if record.is_coding_step && state.executions.values().any(|e| e.is_coding_step && e.lifecycle.holds_capacity()) { return Ok(false); }
...
pub fn record_execution_spawn(state: &mut TaskExecutionState, id: &str, session_id: &str, result: Result<(), String>) -> Result<(), String> {
    ...
    if record.lifecycle != ExecutionLifecycle::Starting { return Err("execution has no launch claim".into()); }
    match result {
        Ok(()) => record.lifecycle = ExecutionLifecycle::Running,
        Err(error) => { record.lifecycle = ExecutionLifecycle::LaunchFailed; record.shutdown_confirmed = true; record.error = Some(error); }
    }
    Ok(())
}
...
pub fn grant_execution_completion(state: &mut TaskExecutionState, id: &str, session_id: &str) -> Result<(), String> {
    let record = owned_execution(state, id, session_id)?;
    if record.lifecycle != ExecutionLifecycle::Running || record.receipt_id.is_some() { return Err("execution is not awaiting completion permission".into()); }
    record.permission = CompletionPermission::HumanGranted { execution_id: id.into(), session_id: session_id.into() };
    Ok(())
}
...
fn assigned_members(repo: &Path, slug: &str, assignment: &OutputAssignment) -> Result<Vec<(String, String)>, String> {
    ...
    let paths = if physical.wildcard {
        let probe = if physical.directory.is_empty() { "__assignment_probe.md".to_string() } else { format!("{}/__assignment_probe.md", physical.directory) };
        ...
        for entry in fs::read_dir(parent).map_err(|e| format!("read assigned output directory: {e}"))? {
            ...
            let relative = if physical.directory.is_empty() { name } else { format!("{}/{name}", physical.directory) };
            if physical.matches(&relative) { paths.push(relative); }
        }
        paths.sort();
        paths
    } else { vec![assignment.relative_path.clone()] };
    if paths.is_empty() { return Err(format!("required output family '{}' is empty", assignment.relative_path)); }
    ...
    for relative in paths {
        ...
        if !meta.is_file() || meta.file_type().is_symlink() || meta.len() == 0 { return Err(format!("required output '{relative}' must be a nonempty regular file")); }
        let logical_path = if physical.wildcard {
            let name = relative.rsplit('/').next().ok_or("invalid output filename")?;
            let slot = name.strip_prefix(&physical.prefix).and_then(|rest| rest.strip_suffix(&physical.suffix)).filter(|s| !s.is_empty()).ok_or("wildcard output member must have a nonempty slot")?;
            let name = format!("{}{}{}", logical.prefix, slot, logical.suffix);
            if logical.directory.is_empty() { name } else { format!("{}/{name}", logical.directory) }
        } else { assignment.selector.clone() };
...
pub fn accept_execution_completion(repo: &Path, slug: &str, state: &mut TaskExecutionState, id: &str, session_id: &str) -> Result<CompletionOutcome, String> {
    let record = owned_execution(state, id, session_id)?;
    if let Some(receipt_id) = &record.receipt_id { return Ok(CompletionOutcome::Accepted { receipt_id: receipt_id.clone() }); }
    if record.lifecycle != ExecutionLifecycle::Running { return Err("execution owner is not running".into()); }
    ...
    for assignment in &record.outputs {
        match assigned_members(repo, slug, assignment) {
            Ok(members) => for (logical_path, relative_path) in members {
                let occurrence_id = uuid::Uuid::new_v4().to_string();
                accepted.push(ArtifactOccurrence {
                    id: occurrence_id, producer_execution_id: Some(id.into()), selector: assignment.selector.clone(), logical_path, relative_path,
                    depth: record.depth, discriminator: assignment.discriminator, context_id: record.candidate.context_id.clone(), collection_ids: BTreeSet::new(),
                });
            },
    ...
    record.error = None;
    for occurrence in accepted { state.occurrences.insert(occurrence.id.clone(), occurrence); }
...
pub fn confirm_execution_exit(state: &mut TaskExecutionState, id: &str, session_id: &str, exit_code: Option<i32>) -> Result<(), String> {
    ...
    record.lifecycle = if record.receipt_id.is_some() { ExecutionLifecycle::Completed } else { ExecutionLifecycle::Failed };
    if record.receipt_id.is_none() { record.error = Some(format!("execution ended without accepted completion (exit {exit_code:?})")); }
    Ok(())
}

pub fn recover_execution_owner(state: &mut TaskExecutionState, id: &str) -> Result<String, String> {
    let record = state.executions.get_mut(id).ok_or("unknown execution")?;
    if record.receipt_id.is_some() || !record.shutdown_confirmed { return Err("recovery requires an unaccepted execution with proven stopped owner".into()); }
...
pub fn execution_assignment_prompt(repo: &Path, slug: &str, state: &TaskExecutionState, id: &str) -> Result<String, String> {
    ...
    for (selector, ids) in &record.candidate.inputs {
        for occurrence_id in ids {
            ...
            let _ = writeln!(prompt, "Read: {} (logical {selector}; occurrence {occurrence_id}; producer {})", root.join(&occurrence.relative_path).display(), occurrence.producer_execution_id.as_deref().unwrap_or("seed"));
        }
    }
    for assignment in &record.outputs {
        let _ = writeln!(prompt, "Write: {} (logical {}; {}required)", root.join(&assignment.relative_path).display(), assignment.selector, if assignment.relative_path.contains('*') { "nonempty wildcard family, all members " } else { "" });
    }
