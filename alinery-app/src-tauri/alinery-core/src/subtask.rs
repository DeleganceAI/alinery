...
fn rollback_created_child(repo: &Path, child_slug: &str, worktree: &Path, meta_path: &Path, meta_bytes: &[u8]) {
...
pub fn create_subtask(repo: &Path, input: CreateSubtaskInput) -> Result<CreateSubtaskResult, String> {
    with_task_mutation_lock(repo, "create sub-task", || {
        ...
        if attempt.is_err() {
            rollback_created_child(repo, &input.slug, &child_worktree, &manager.meta_path, &meta_bytes);
        }
        attempt
    })
}
...
mod tests {
    ...
    fn subtask_creation_from_dirty_parent_keeps_changes_parent_only() {
        ...
        assert_eq!(read_task_relationships(&repo, &parent.slug).unwrap().active_subtasks[0].slug, child.slug);
    ...
    fn subtask_creation_allows_second_worktree_child_from_second_manager() {
        ...
        let first = create_subtask(&repo, create_input()).unwrap();
        assert_eq!(first.child_task.slug, "b");
        ...
        assert_eq!(second.child_task.slug, "c");
